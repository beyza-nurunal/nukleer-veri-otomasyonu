...
def yazar_yil_bul(dosya_yolu):
    ...
    if not yazar:
        ...
        for kelime in kelimeler:
            if len(kelime) > 2 and kelime.lower() not in ['txt', 'dat', 'asc', 'out', 'csv', 'mev', 'data']:
    ...
    if yil: return f"{yazar} vd. {yil}"
    else: return f"{yazar} vd."
...
legacy y
"""
        inp_dosya_yolu = os.path.join(klasor_adi, f"input_ld{ld}.inp")
...
def veri_oku(dosya_yolu):
    df = pd.read_csv(dosya_yolu, sep=r"\s+", comment="#", header=None, engine="python")
    toplam = df.shape[1]
    if toplam == 2: df = df.iloc[:, [0, 1]]
    elif toplam == 3: df = df.iloc[:, [0, 2]]
    else: df = df.iloc[:, [0, 2, 3]]
...
merkez_hizalama = Alignment(horizontal="center", vertical="center")

islenen_klasor_sayisi = 0

for reaksiyon in reaksiyonlar:
    dosyalar = [d for d in os.listdir(reaksiyon) if d.endswith((".txt", ".dat", ".asc", ".out"))]
    ...
    for dosya in dosyalar:
        ...
        ws.cell(row=1, column=current_col, value=baslik_yazisi)
        ws.merge_cells(start_row=1, start_column=current_col, end_row=1, end_column=current_col + sutun_sayisi - 1)
        ws.cell(row=1, column=current_col).alignment = merkez_hizalama
        ws.cell(row=1, column=current_col).font = kalin_font
        
        alt_basliklar = ["Enerji (MeV)", "Data (mb)", "Hata (mb)"]
        for i in range(sutun_sayisi):
            baslik = alt_basliklar[i] if i < len(alt_basliklar) else f"Sutun {i+1}"
            c = ws.cell(row=2, column=current_col + i, value=baslik)
        ...
        for row_idx, row_data in enumerate(df.values):
            for col_idx, val in enumerate(row_data):
                if pd.notna(val):
                    ws.cell(row=row_idx + 3, column=current_col + col_idx, value=val)
        
        ortak_enerjiler.extend(df.iloc[:, 0].dropna().tolist())
        current_col += sutun_sayisi
        
    # SARI SUTUN (Ortak Enerjiler)
    ...
    for row_idx, val in enumerate(ortak_enerjiler):
        c = ws.cell(row=row_idx + 3, column=current_col, value=val)
        c.fill = sari_dolgu
    ...
    for ld in range(1, ld_sayisi + 1):
        c1 = ws.cell(row=1, column=current_col, value=f"TALYS 2.0 (AOMP{ld})")
        c1.alignment = merkez_hizalama
        c1.font = Font(bold=False)
        
        c2 = ws.cell(row=2, column=current_col, value="Enerji (MeV) / Data (mb)")
    ...
    talys_dosyalari_olustur(reaksiyon, ld_sayisi, ortak_enerjiler)
